...
def handleTransfer(dao):
    ...
    dc_rows = ['delegate', 'delegator', 'previousDelegate', 'blockNumber']
    dvpc_rows = ['delegate', 'previousBalance', 'newBalance', 'blockNumber']
    ...
    #  create res/txnHash folder if it doesn't exist
    if not os.path.exists(f"./res/txnHash"):
        os.makedirs(f"./res/txnHash")

    # Save the combined dataframe to a new CSV file
    combined_df.to_csv(f'./res/combined/{dao}.csv', index=False)

# handle block time and merge rows for transfers
