...
def handle_spells(path, jsonData):
        ...
        spells = pd.json_normalize(jsonData["spells"], record_path='timeLine', meta=['id', 'timestamp', 'approvals', 'casted', 'castedWith', 'lifted', 'liftedWith'], record_prefix='_')
        
        convert_dict ={'_wad': float,
                       '_locked': float}

        spells = spells.astype(convert_dict)
        spells['_wad'].fillna(0, inplace=True)
        spells['_locked'].fillna(0, inplace=True)
        spells['weight'] = spells['_wad'] + spells['_locked']
